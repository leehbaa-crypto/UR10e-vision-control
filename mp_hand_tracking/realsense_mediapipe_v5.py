...
class UnifiedVisionNodeV5(Node):
    def __init__(self):
        super().__init__('nrs_vision_node_v5')
        ...
        self.get_logger().info("✅ Vision Node V5 Started (Proportional Pad Updated)")
    ...
    def camera_loop(self):
        # [기능 2 반영] 실제 로봇 작업 공간(Y: 0.6m, X: 0.3m) 비율인 2:1에 맞추어 패드 크기를 400x200으로 크게 확장
        ...
        while self.is_running and rclpy.ok():
            ...
            if self.robot_state == "NOT READY":
                cv2.putText(display, f"L-POINT to CHANGE: {self.surfaces[self.current_idx]}", (10, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                cv2.putText(display, "Make Double 'OK' to Launch", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            ...
            if self.robot_state == "GUIDED_FOLLOWING":
                ...
                cv2.putText(display, "TOP VIEW PAD", (flipped_pad[0], flipped_pad[1]-25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
                cv2.putText(display, f"[{self.surfaces[self.current_idx]}]", (flipped_pad[0], flipped_pad[1]-8), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)

            cv2.imshow('Vision V5 (Proportional Pad)', display)
...
def main(args=None):
    rclpy.init(args=args); node = UnifiedVisionNodeV5()
    try: rclpy.spin(node)
