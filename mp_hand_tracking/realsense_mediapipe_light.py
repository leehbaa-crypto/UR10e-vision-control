...
class UnifiedVisionNodeLight(Node):
    def __init__(self):
        super().__init__('nrs_vision_node_light')
        ...
        self.get_logger().info("✅ LIGHT Vision Node Started (v5 logic preserved)")
    ...
    def camera_loop(self):
        ...
        while self.is_running and rclpy.ok():
                    ...
                    if label == "Right": 
                        ...
                        if self.robot_state == "FOLLOWING":
                            if 0 <= cx < 640 and 0 <= cy < 480:
                                d_list = []
                                for dx, dy in [(-1,0), (1,0), (0,-1), (0,1), (0,0)]: # ⚡ 경량화 4: 샘플링 개수 최적화
            ...
            if self.robot_state == "NOT READY":
                ...
                cv2.putText(display, "Double 'OK' to Launch", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            ...
            cv2.imshow('Vision LIGHT (i3 Optimized)', display)
...
def main(args=None):
    rclpy.init(args=args); node = UnifiedVisionNodeLight()
    try: rclpy.spin(node)
