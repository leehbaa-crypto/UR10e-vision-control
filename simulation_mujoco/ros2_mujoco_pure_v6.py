...
class MuJoCoDynamicPhysicsBridgeV9(Node):
    def __init__(self):
        super().__init__('nrs_dynamic_physics_bridge_v9')
...
"""
        ...
        # 💡 [초강력 파라미터 적용] 바위처럼 단단하고 떨림 없는 극한의 관절 강성
        self.kp = np.array([30000, 30000, 20000, 10000, 5000, 5000]) 
        self.kv = np.array([1500,  1500,  1000,   500,   200,  200]) 
        self.torque_limits = np.array([3000.0]*6)
        ...
        self.get_logger().info(f"✅ MuJoCo Bridge V9 (Rigid Dynamics + 60Hz Render Sync) Ready!")
...
def main(args=None):
    rclpy.init(args=args); node = MuJoCoDynamicPhysicsBridgeV9()
    try: node.run()
