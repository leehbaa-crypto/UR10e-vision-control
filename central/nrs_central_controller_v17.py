...
class CentralControllerV21(Node):
    def __init__(self):
        super().__init__('nrs_central_controller_fixed')
        ...
        self.tool_len = 0.145 
        # 💡 물리 엔진에 맞게 표면을 1cm 확실히 짓누르도록 설정 (닿지 않는 현상 해결)
        ...
        self.get_logger().info("🌟 Central Controller (V14 Logic Restored & Strict Lock Applied) Ready.")
    ...
    def guided_cb(self, msg):
        if self.current_state == "GUIDED_FOLLOWING":
            self.last_valid_time = time.time() 
            ...
            # 💡 [V14 복구] 타겟 필터를 살려 좌우 움직임과 하강을 모두 부드럽게 유도합니다.
            safe_target = np.array([rx, ry, target_z])
    ...
    def gesture_cb(self, msg):
        ...
        if len(self.gesture_history) == 10:
            ...
            # 💡 [핵심] 철통 잠금장치: IDLE이나 ESTOP 상태에서만 다른 모드로 넘어갈 수 있습니다.
            # 작업 도중 제스처가 튀어도 절대 모드가 풀리지 않습니다!
            if count["rock_rock"] >= 7: 
                if self.current_state not in ["ESTOP", "RESETTING", "NOT READY"]:
                    self.resume_state = self.current_state
                self.current_state = "ESTOP"
                
            elif count["paper_paper"] >= 7 and self.current_state in ["IDLE", "ESTOP"]: 
    ...
    def solve_ik_safe(self, q_seed, target_pos, target_z_dir):
        # 💡 [V14 복구] 억지 수식을 걷어낸 가장 가볍고 안정적인 IK 로직
        q = q_seed.copy()
    ...
    def control_loop(self):
        ...
        # 💡 [V14 완벽 복구] FOLLOWING과 GUIDED_FOLLOWING이 이 완벽한 3D 이동 블록을 공유합니다.
            ...
            if time.time() - self.last_valid_time > 1.5:
                ...
                max_step = 0.02 # 너무 빠르지도 느리지도 않은 최적의 속도
...
def main():
    rclpy.init(); node = CentralControllerV21()
    try: rclpy.spin(node)
