...
class CentralControllerV13(Node):
    def __init__(self):
        super().__init__('nrs_central_controller_v13')
        ...
        # 💡 [관통 방지] 로봇 끝이 표면을 파고들지 않도록 2cm(0.02m) 띄워서 닿게 만듭니다.
        self.tcp_z_offset = 0.02 
        ...
        self.get_logger().info("🌟 Central Controller V13 (Inversion Applied & Penetration Fixed) Ready.")
    ...
    def guided_cb(self, msg):
        if self.current_state == "GUIDED_FOLLOWING" and self.mesh is not None:
            ...
            # 💡 정상적인 표면 법선(Normal) 방향으로 tcp_z_offset(2cm) 만큼 띄워 관통을 방지합니다.
    ...
    def solve_ik_safe(self, q_seed, target_pos, target_z_dir):
        """ 💡 [핵심 이식 완료] 디버그 코드에서 성공했던 -X, -Y 좌표 반전을 완벽 이식 """
        q = q_seed.copy()
        
        # World 프레임 -> IK 프레임으로 180도 뒤집어서 입력
    ...
    def control_loop(self):
        ...
        if self.needs_ik_reset:
            # IK 엔진이 뱉어낸 거꾸로 된 좌표를 World 프레임으로 다시 돌려놓습니다.
            ...
            self.target_pose = world_curr_pos.copy()
            self.target_z_dir = world_curr_z_dir.copy()
        ...
        if self.current_state in ["FOLLOWING", "GUIDED_FOLLOWING", "DEBUG_VERTICAL"]:
            if self.current_state in ["FOLLOWING", "GUIDED_FOLLOWING"] and time.time() - self.last_valid_time > 1.5:
                ...
                # 이전 정답인 self.target_q를 시드로 주어 끊김 없이 움직이게 만듭니다.
...
def main():
    rclpy.init(); node = CentralControllerV13()
    try: rclpy.spin(node)
